...
class DatasetProcessing(object):
    ...
    def getSingleImage(self, img_array):
        #image = misc.imread(self.pathToImgs)
        image_resized = resize(img_array, (18,22), mode='reflect')
        img_array = []
        ...
        for image_num in range(len(img_array)):
            for rownum in range(len(img_array[image_num])):
                for colnum in range(len(img_array[image_num][rownum])):
                    grey[image_num][rownum][colnum] = self.weightedAverage(img_array[image_num][rownum][colnum])
        final_img = np.zeros(shape=396)
        final_img = grey[0].reshape(1, 396)
        return final_img
